...
def update_user_memory(clientName,newspace):
    ...
    with open(root + '/' + clientName + '.json','r+') as load:
        ...
        userCon['used_space'] = (float)(userCon['total_size']) - newspace
    with open(root + '/' + clientName + '.json','w+') as load:
...
def client_thread(clientsocket, ip, port,serverID , MAX_BUFFER = 4096):       # MAX_BUFFER_SIZE is how big the message can be
    ...
    if not os.path.exists(root+'/'+clientName):
    ...
    while True:
        ...
        with Switch(option) as case:
            ...
            if case(b'0110'):
                ...
                try:
                    #try remove file
                    removed = (os.path.getsize(current_location +'/'+ fileName))/(1024*1024)
                    os.remove(current_location +'/'+ fileName)
                    current_free_space += removed 
                    update_user_memory(clientName, current_free_space)
                    clientsocket.sendall(SUCCCESS)
                except OSError as e:
            ...
            if case(b'1000'):
                try:
                    ...
                    print(current_location[:current_location.rfind('/')])
                    current_location = current_location[:current_location.rfind('/')]
                    clientsocket.sendall(SUCCCESS)
                except OSError as e:
                    error_replay(clientsocket, e.strerror)
